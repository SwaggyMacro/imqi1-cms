...
AGENTS.md：根目录**没有** lint/type-check 脚本，必须回到项目根再用 `bunx eslint .` / `bunx nuxi typecheck` / `bun run tailwindcss:lint`；CWD 残留在 `mini/` 会验错项目（mini 的脚本在根，用 `bun run mini:*`）。本流程**先把目录校正回项目根**再跑，这是最关键的一步。
