---
name: deploy
description: 构建并上传到云端：bun run build + upload:cos + upload:server。**先校验 .env 里 COS/SERVER 上传配置是否齐全（缺即停）**；upload:cos 有交互式「清空远程目录 yes/no」——Codex 可以用 `printf 'y\n' |` 喂 stdin 代跑，但必须先得到用户明确确认。清空范围以 `buildHashPrefix`（.build-hash-dir，配 CDN 时恒为 `static/<hash>`）为准，当前配置下 `y` 只清本次构建目录、非全站；**`n` 在前缀已有文件时会取消整个上传（exit 0）**，须向用户取明确 y/n。每次改动后要发版部署时使用。
---
...
> 若用户坚持自己跑，可贴 `! bun run upload:cos` 让其当面回答；Codex 不要假设「喂不了 stdin」，但仍不能替用户决定是否清空远程目录。
