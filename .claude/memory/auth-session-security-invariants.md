...
description: auth.ts sessionId/authCode 必须用 crypto.randomBytes(非 Math.random);登录响应白名单;login-rate-limit 是按 IP 限流(Redis 优先+内存兜底)
metadata: 
  node_type: memory
  type: project
...
3. **`server/utils/login-rate-limit.ts` 是按 IP 的限流器,Redis 优先、内存兜底**(窗口 5 次/15 分,锁 15 分)。**有 Redis 时用 `INCR`+`EXPIRE` 持久计数——跨重启、多实例共享**;Redis 未配置或抛异常才回落进程内存 Map(重启清零、单实例),Redis 异常被 try/catch 兜住**不阻断登录**。`login.post.ts` 在 CSRF 之前调 `checkLoginRateLimit`(洪水请求最早被拒),失败调 `recordLoginFailure`,成功调 `resetLoginAttempts`;判定是否要「自适应验证码」用 `hasRecentFailures`。新增类似鉴权端点(2FA、改密、找回)应复用此 util。(**2026-09-14 订正**:早先记的「纯内存 Map、多实例计数独立」已不成立——限流已迁 Redis 优先。)
...
6. **登录页 `redirectTo` 只允许站内绝对路径**(`login.vue`):条件为**正则白名单** `/^\/[a-zA-Z0-9/?#&=_%.\-:@~]+$/` **且** `!to.startsWith('//')` **且** `!to.includes('\\')`,否则回落 `/admin`。白名单是为了拒控制字符(`%00` 等)、空白、反斜杠这类**浏览器规范化差异绕过**(如 `/\evil.com`、含 `\n\t` 的 payload);`//` 与 `\` 两条是双保险。Nuxt navigateTo 本已挡外链,这是防御性二次清洗。(**2026-09-14 订正**:早先记的 `startsWith('/')` 判断已升级为上述正则白名单。)
